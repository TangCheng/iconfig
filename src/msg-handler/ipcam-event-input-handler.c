...
#include "ipcam-event-input-handler.h"
#include "iconfig.h"
#include "common.h"
#include "sysutils.h"
#include "database/event_input.h"

G_DEFINE_TYPE (IpcamEventInputMsgHandler, ipcam_event_input_msg_handler, IPCAM_TYPE_MESSAGE_HANDLER);

static void
ipcam_event_input_msg_handler_init (IpcamEventInputMsgHandler *ipcam_event_input_msg_handler)
{
}

static void
ipcam_event_input_msg_handler_finalize (GObject *object)
{
    G_OBJECT_CLASS (ipcam_event_input_msg_handler_parent_class)->finalize (object);
}
...
static gboolean
ipcam_event_input_msg_handler_put_action_impl(IpcamMessageHandler *handler, JsonNode *request, JsonNode **response)
{
    JsonBuilder *builder = json_builder_new();
    JsonObject *req_obj;
    GList *members, *item;
    ...
    for (item = g_list_first(members); item; item = g_list_next(item))
    {
            ...
            g_str_equal(name, "input2"))
        {
            ...
            ipcam_event_input_msg_handler_update_param(IPCAM_EVENT_INPUT_MSG_HANDLER(handler), name, value_obj);
            json_builder_set_member_name(builder, name);
            json_builder_begin_object(builder);
            ipcam_event_input_msg_handler_read_param(IPCAM_EVENT_INPUT_MSG_HANDLER(handler), builder, name);
            json_builder_end_object(builder);
...
static void
ipcam_event_input_msg_handler_class_init (IpcamEventInputMsgHandlerClass *klass)
{
    GObjectClass* object_class = G_OBJECT_CLASS (klass);
    IpcamMessageHandlerClass *parent_class = IPCAM_MESSAGE_HANDLER_CLASS(klass);

    object_class->finalize = ipcam_event_input_msg_handler_finalize;

    parent_class->get_action = ipcam_event_input_msg_handler_get_action_impl;
    parent_class->put_action = ipcam_event_input_msg_handler_put_action_impl;
}
