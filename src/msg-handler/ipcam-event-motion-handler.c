...
#include "ipcam-event-motion-handler.h"
#include "iconfig.h"
#include "common.h"
#include "sysutils.h"
#include "database/event_motion.h"

G_DEFINE_TYPE (IpcamEventMotionMsgHandler, ipcam_event_motion_msg_handler, IPCAM_TYPE_MESSAGE_HANDLER);

static void
ipcam_event_motion_msg_handler_init (IpcamEventMotionMsgHandler *ipcam_event_motion_msg_handler)
{
}

static void
ipcam_event_motion_msg_handler_finalize (GObject *object)
{
    G_OBJECT_CLASS (ipcam_event_motion_msg_handler_parent_class)->finalize (object);
}

static gboolean
ipcam_event_motion_msg_handler_read_param(IpcamEventMotionMsgHandler *handler, JsonBuilder *builder, const gchar *name)
{
    ...
    value = ipcam_iconfig_read(iconfig, IPCAM_EVENT_MOTION_TYPE, name, "enable");
    if (value)
    {
    ...
    value = ipcam_iconfig_read(iconfig, IPCAM_EVENT_MOTION_TYPE, name, "sensitivity");
    if (value)
    {
    ...
    value = ipcam_iconfig_read(iconfig, IPCAM_EVENT_MOTION_TYPE, name, "rect");
    if (value)
    {
        Rect *rect;
        if (IS_64BIT_MACHINE)
    ...
    value = ipcam_iconfig_read(iconfig, IPCAM_EVENT_MOTION_TYPE, name, "schedules");
    if (value)
    {
        Schedules *sche;
        if (IS_64BIT_MACHINE)
...
static gboolean
ipcam_event_motion_msg_handler_update_param(IpcamEventMotionMsgHandler *handler, const gchar *name, JsonObject *value_obj)
{
    IpcamIConfig *iconfig;
    g_object_get(G_OBJECT(handler), "app", &iconfig, NULL);
    GList *members, *item;
    ...
    for (item = g_list_first(members); item; item = g_list_next(item))
    {
        ...
        if (value)
        {
            ret = ipcam_iconfig_update(iconfig, IPCAM_EVENT_MOTION_TYPE, name, sub_name, value);
            g_variant_unref(value);
            value = NULL;
        }
    }
...
static gboolean
ipcam_event_motion_msg_handler_get_action_impl(IpcamMessageHandler *handler, JsonNode *request, JsonNode **response)
{
    ...
    for (i = 0; i < json_array_get_length(req_array); i++)
    {
            ...
            g_str_equal(name, "region2"))
        {
            ...
            json_builder_begin_object(builder);
            ipcam_event_motion_msg_handler_read_param(IPCAM_EVENT_MOTION_MSG_HANDLER(handler), builder, name);
            json_builder_end_object(builder);
        }
    }
    json_builder_end_object(builder);
...
static gboolean
ipcam_event_motion_msg_handler_put_action_impl(IpcamMessageHandler *handler, JsonNode *request, JsonNode **response)
{
    JsonBuilder *builder = json_builder_new();
    JsonObject *req_obj;
    GList *members, *item;
    ...
    for (item = g_list_first(members); item; item = g_list_next(item))
    {
            ...
            g_str_equal(name, "region2"))
        {
            ...
            ipcam_event_motion_msg_handler_update_param(IPCAM_EVENT_MOTION_MSG_HANDLER(handler), name, value_obj);
            json_builder_set_member_name(builder, name);
            json_builder_begin_object(builder);
            ipcam_event_motion_msg_handler_read_param(IPCAM_EVENT_MOTION_MSG_HANDLER(handler), builder, name);
            json_builder_end_object(builder);
...
static void
ipcam_event_motion_msg_handler_class_init (IpcamEventMotionMsgHandlerClass *klass)
{
    GObjectClass* object_class = G_OBJECT_CLASS (klass);
    IpcamMessageHandlerClass *parent_class = IPCAM_MESSAGE_HANDLER_CLASS(klass);

    object_class->finalize = ipcam_event_motion_msg_handler_finalize;

    parent_class->get_action = ipcam_event_motion_msg_handler_get_action_impl;
    parent_class->put_action = ipcam_event_motion_msg_handler_put_action_impl;
}
