...
typedef struct _IpcamEventMotionPrivate
{
    guint id;
    gchar *name;
    gboolean enable;
    ...
    gchar *saturday;
    gchar *sunday;
} IpcamEventMotionPrivate;

G_DEFINE_TYPE_WITH_PRIVATE(IpcamEventMotion, ipcam_event_motion, GOM_TYPE_RESOURCE);

static GParamSpec *obj_properties[N_PROPERTIES] = {NULL, };

static void ipcam_event_motion_finalize(GObject *object)
{
    IpcamEventMotionPrivate *priv = ipcam_event_motion_get_instance_private(IPCAM_EVENT_MOTION(object));
    g_free(priv->name);
    g_free(priv->monday);
    g_free(priv->tuesday);
    ...
    g_free(priv->sunday);
    G_OBJECT_CLASS(ipcam_event_motion_parent_class)->finalize(object);
}
static void ipcam_event_motion_set_property(GObject      *object,
                                           guint        property_id,
                                           const GValue *value,
                                           GParamSpec   *pspec)
{
    IpcamEventMotion *self = IPCAM_EVENT_MOTION(object);
    IpcamEventMotionPrivate *priv = ipcam_event_motion_get_instance_private(self);
    switch(property_id)
    {
    case PROP_ID:
...
static void ipcam_event_motion_get_property(GObject    *object,
                                           guint       property_id,
                                           GValue     *value,
                                           GParamSpec *pspec)
{
...
static void ipcam_event_motion_init(IpcamEventMotion *self)
{
}
static void ipcam_event_motion_class_init(IpcamEventMotionClass *klass)
{
    GObjectClass *object_class = G_OBJECT_CLASS(klass);
    object_class->set_property = &ipcam_event_motion_set_property;
    object_class->get_property = &ipcam_event_motion_get_property;
    object_class->finalize = &ipcam_event_motion_finalize;

    GomResourceClass *resource_class = GOM_RESOURCE_CLASS(klass);
    gom_resource_class_set_table(resource_class, "event_motion");
    ...
    obj_properties[PROP_LEFT] =
        g_param_spec_uint("left",
                          "left",
                          "motion detect region left position.",
                          0,
                          1000,
                          0, // default value
                          G_PARAM_READWRITE);
    obj_properties[PROP_TOP] =
        g_param_spec_uint("top",
                          "top",
                          "motion detect region top position.",
                          0,
                          1000,
                          0, // default value
                          G_PARAM_READWRITE);
