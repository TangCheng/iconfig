...
static gchar* do_get_action(IpcamIConfig *iconfig, GList *item_list)
{
    ...
    IpcamMessageHandler *msg_handler = g_object_new(IPCAM_TYPE_NETWORK_MSG_HANDLER,
                                                      "app", iconfig, NULL);

    ipcam_message_handler_do_get(msg_handler, "get_network", req_node, &res_node);
...
START_HANDLER(get_network, HTTP_GET, "/api/1.0/network.json", http_request, http_response)
{
    ...
    g_object_get(get_network, "app", &iconfig, NULL);
...
static gchar* do_put_action(IpcamIConfig *iconfig, JsonNode *request)
{
    ...
    ipcam_message_handler_do_put(msg_handler, "set_network", request, &response);
...
START_HANDLER(put_network, HTTP_PUT, "/api/1.0/network.json", http_request, http_response)
{
    gchar *body = NULL;
    IpcamIConfig *iconfig;
    gboolean success = FALSE;

    g_object_get(put_network, "app", &iconfig, NULL);
