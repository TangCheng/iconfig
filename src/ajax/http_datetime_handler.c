...
static gchar* do_get_action(IpcamIConfig *iconfig, GList *item_list)
{
    ...
    IpcamMessageHandler *msg_handler = g_object_new(IPCAM_TYPE_DATETIME_MSG_HANDLER,
                                                      "app", iconfig, NULL);

    ipcam_message_handler_do_get(msg_handler, "get_datetime", req_node, &res_node);
...
START_HANDLER(get_datetime, HTTP_GET, "/api/1.0/datetime.json", http_request, http_response)
{
    ...
    g_object_get(get_datetime, "app", &iconfig, NULL);
...
static gchar* do_put_action(IpcamIConfig *iconfig, JsonNode *request)
{
    ...
    ipcam_message_handler_do_put(msg_handler, "set_datetime", request, &response);
...
START_HANDLER(put_datetime, HTTP_PUT, "/api/1.0/datetime.json", http_request, http_response)
{
    gchar *body = NULL;
    IpcamIConfig *iconfig;
    gboolean success = FALSE;

    g_object_get(put_datetime, "app", &iconfig, NULL);
